...
// 初始化状态
const DEFAULT_CLIENT_ID = '3f2504e04f8911d39a0c0305e82c3301'
...
export const useTokenStore = defineStore(
  ...
  () => {
    ...
    const clientId = ref<string>(DEFAULT_CLIENT_ID)
    const setClientId = (val: string) => {
      clientId.value = val
    }
    ...
    const login = async (loginForm: ILoginForm) => {
      try {
        ...
        const cid = raw?.clientid ?? raw?.client_id ?? raw?.clientId
        if (cid) {
          setClientId(cid)
        }
        let normalized: IAuthLoginRes = res as any
        if (raw && 'access_token' in raw) {
          if (!isDoubleTokenMode) {
            normalized = { token: raw.access_token, expiresIn: raw.expire_in || 0 }
          }
          else {
            normalized = {
              accessToken: raw.access_token,
              refreshToken: raw.refresh_token || '',
              accessExpiresIn: raw.expire_in || raw.access_expires_in || 0,
              refreshExpiresIn: raw.refresh_expire_in || 0,
            }
          }
        }
        await _postLogin(normalized)
        uni.showToast({
          title: '登录成功',
          icon: 'success',
        })
    ...
    /**
     * 微信登录
     * 使用完整的微信小程序登录流程，后端返回token和用户信息
     * @returns 登录结果
     */
    const wxLogin = async () => {
      try {
        // 使用完整的微信小程序登录流程
        const res = await wechatMiniLoginFlow()
        console.log('微信登录-res: ', res)

        // 设置token信息
        setTokenInfo(res)

        // 如果后端返回了用户信息，直接设置，否则再次获取
        const userStore = useUserStore()
        if (res.user) {
          const hasRoles = Array.isArray((res.user as any).roles) && (res.user as any).roles.length > 0
          if (!hasRoles) {
            await userStore.fetchUserInfo()
          }
          else {
    ...
    const refreshToken = async () => {
      ...
      try {
        ...
        let normalized: IDoubleTokenRes = res as any
        const raw: any = res as any
        if (raw && 'access_token' in raw) {
          normalized = {
            accessToken: raw.access_token,
            refreshToken: raw.refresh_token || tokenInfo.value.refreshToken,
            accessExpiresIn: raw.expire_in || raw.access_expires_in || 0,
            refreshExpiresIn: raw.refresh_expire_in || 0,
          }
        }
