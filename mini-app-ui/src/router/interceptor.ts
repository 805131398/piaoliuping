  ...
  invoke({ url, query }: { url: string, query?: Record<string, string> }) {
    ...
    if (isNeedLoginMode) {
      ...
      // 否则需要重定向到登录页
        ...
        FG_LOG_ENABLE && console.log('1 isNeedLogin(白名单策略) redirect to Me with:', redirectQuery)
    ...
    // #region 2/2 默认不需要登录的情况(黑名单策略) ---------------------------
      ...
      if (judgeIsExcludePath(path)) {
        FG_LOG_ENABLE && console.log('2 isNeedLogin(黑名单策略) redirect to Me with:', redirectQuery)
        toLoginPage({ queryString: redirectQuery })
